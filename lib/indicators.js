 ...
 * @param {boolean} detectCrossovers - Whether to detect and flag crossovers
 * @returns {Array} Array of percentage deviations from moving average
 */
function calculatePriceRelativeToMA(data, period, detectCrossovers = false) {
  ...
  // If detectCrossovers is true, check for crossovers and return them
  if (detectCrossovers && data.length >= period + 1) {
    const crossovers = Array(data.length).fill(0);
    
    for (let i = period; i < data.length; i++) {
      // Check for crossover (when price crosses the MA line)
      if (!isNaN(result[i]) && !isNaN(result[i-1])) {
        // Bullish crossover: below MA to above MA (negative to positive)
        if (result[i-1] < 0 && result[i] >= 0) {
          crossovers[i] = 1; // Bullish crossover (crosses above)
        }
        // Bearish crossover: above MA to below MA (positive to negative)
        else if (result[i-1] > 0 && result[i] <= 0) {
          crossovers[i] = -1; // Bearish crossover (crosses below)
        }
      }
    }
